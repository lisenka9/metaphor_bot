...
async def handle_random_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обрабатывает случайные текстовые сообщения (не команды)"""
    # Проверяем, что это текстовое сообщение (не команда, не callback)
    if update.message and update.message.text:
        user_message = update.message.text.strip()
        
        # Игнорируем сообщения, которые начинаются с "/" (это команды)
        if user_message.startswith('/'):
            return
        
        # Игнорируем очень короткие сообщения (возможно, опечатки)
        if len(user_message) < 2:
            return
        
        # Проверяем, не находится ли пользователь в процессе заполнения формы
        if 'consult_form' in context.user_data:
            # Если пользователь заполняет форму консультации, передаем управление форме
            await handle_consult_form(update, context)
            return
        
        # ✅ ОТПРАВЛЯЕМ СПРАВКУ ДЛЯ ЛЮБЫХ СЛУЧАЙНЫХ СООБЩЕНИЙ
